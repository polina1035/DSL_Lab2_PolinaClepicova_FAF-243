...
Handling Non‑determinism in the Data Structure
Using a dictionary with keys (state, symbol) and values as sets of target states naturally supports NFA transitions. This simplified both the determinism check and the subset construction.

Subset Construction and State Naming
Representing DFA states as frozenset objects was elegant but required careful handling of the empty set (dead state). The dead state must be explicitly added to ensure completeness.

Grammar Classification
Implementing a robust classify() method that checks for Type‑1 constraints (context‑sensitive) is non‑trivial. For this lab, focusing on the regular grammar case was sufficient.

Graphical Output
Generating DOT code was straightforward, but rendering it required an external tool (Graphviz). The report includes the DOT source; the actual image can be produced separately.
